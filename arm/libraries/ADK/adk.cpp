...
uint32_t ADK::Init(uint32_t parent, uint32_t port, uint32_t lowspeed)
{
	...
	memset(&buf, 0, sizeof(USB_DEVICE_DESCRIPTOR));

	// Get memory address of USB device address pool
	AddressPool	&addrPool = pUsb->GetAddressPool();

	TRACE_USBHOST(Serial1.printf("ADK::Init\r\n");)
...
//	p->epinfo->maxPktSize = 64;
	...
	TRACE_USBHOST(Serial1.printf("dev desc bLength=%lu\n", buf.bLength);)
	TRACE_USBHOST(Serial1.printf("dev desc idVendor=%04X\n", buf.idVendor);)
	TRACE_USBHOST(Serial1.printf("dev desc idProduct=%04X\n", buf.idProduct);)

	// Allocate new address according to device class
	bAddress = addrPool.AllocAddress(parent, false, port);

	// Extract Max Packet Size from device descriptor
	epInfo[0].maxPktSize = buf.bMaxPacketSize0;
	...
	if (rcode)
	{
		...
		bAddress = 0;
		TRACE_USBHOST(Serial1.printf("ADK::Init : setAddr failed with rcode %lu\r\n", rcode);)
		return rcode;
	}

	TRACE_USBHOST(Serial1.printf("ADK::Init : device address is now %lu\r\n", bAddress);)
	...
	if (buf.idVendor == ADK_VID &&
		(buf.idProduct == ADK_PID || buf.idProduct == ADB_PID))
	{
		TRACE_USBHOST(Serial1.printf("ADK::Init : Accessory mode device detected\r\n");)

		/* Go through configurations, find first bulk-IN, bulk-OUT EP, fill epInfo and quit */
		num_of_conf = buf.bNumConfigurations;

		TRACE_USBHOST(Serial1.printf("ADK::Init : number of configuration is %lu\r\n", num_of_conf);)
		...
		for (uint32_t i = 0; i < num_of_conf; ++i)
		{
			...
			if (rcode)
			{
				TRACE_USBHOST(Serial1.printf("ADK::Init : Got 1st bad code for config: %lu\r\n", rcode);)
		...
		TRACE_USBHOST(Serial1.printf("ADK::Init : ADK device configured successfully\r\n");)
		ready = true;
	...
	TRACE_USBHOST(Serial1.printf("ADK::Init : DK protocol rev. %lu\r\n", adkproto);)
	...
	// Diagnostic messages
FailGetDevDescr:
	TRACE_USBHOST(Serial1.printf("ADK::Init getDevDescr : ");)
	goto Fail;

FailSetDevTblEntry:
	TRACE_USBHOST(Serial1.printf("ADK::Init setDevTblEn : ");)
	goto Fail;

FailGetProto:
	TRACE_USBHOST(Serial1.printf("ADK::Init getProto : ");)
	goto Fail;

FailSwAcc:
	TRACE_USBHOST(Serial1.printf("ADK::Init swAcc : ");)
	goto Fail;

SwAttempt:
	TRACE_USBHOST(Serial1.printf("ADK::Init Accessory mode switch attempt : ");)
	goto Fail;

FailGetConfDescr:
	TRACE_USBHOST(Serial1.printf("ADK::Init getConf : ");)
	goto Fail;

FailSetConf:
	TRACE_USBHOST(Serial1.printf("ADK::Init setConf : ");)
	goto Fail;

Fail:
	TRACE_USBHOST(Serial1.printf("error code: %lu\r\n", rcode);)
	Release();
	return rcode;
}
...
void ADK::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep)
{
	...
	TRACE_USBHOST(Serial1.printf("ADK::EndpointXtract : Found new endpoint\r\n");)
	TRACE_USBHOST(Serial1.printf("ADK::EndpointXtract : deviceEpNum: %lu\r\n", epInfo[index].deviceEpNum);)
	TRACE_USBHOST(Serial1.printf("ADK::EndpointXtract : maxPktSize: %lu\r\n", epInfo[index].maxPktSize);)
	TRACE_USBHOST(Serial1.printf("ADK::EndpointXtract : index: %lu\r\n", index);)
